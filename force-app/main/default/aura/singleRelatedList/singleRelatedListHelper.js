({
  ...
  fireMassAction: function (cmp) {
    var appEvent = $A.get("e.c:EventTransferJSON");
    var selectedRecords = cmp.get('v.selectedRecords');
    if(selectedRecords.length > 0 ){
      appEvent.setParams({ "json" : JSON.stringify(selectedRecords), "dataType":cmp.get("v.sobjectApiName")});
      appEvent.fire();
    }
  }
})